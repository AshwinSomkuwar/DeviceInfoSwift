...
// MARK: UIDevice & String Extension
extension String {
...
public extension UIDevice {
    ...
    var type: Model {
        ...
        guard let mcode = modelCode?.utf8EncodedString(), let model = modelMap[mcode] else { return Model.unrecognized }
        if model == .simulator {
            if let simModelCode = ProcessInfo().environment["SIMULATOR_MODEL_IDENTIFIER"] {
                if let simModel = modelMap[simModelCode.utf8EncodedString()] {
                    return simModel
                }
            }
        }
